...
- **กราฟ:** เป็น SVG ที่ประกอบเองผ่าน helper ใน `chart.ts` (อย่าเพิ่ม charting lib). กราฟที่ซูม/เลื่อนได้ (`ProductionCurve`, `SolarProductionRateCurve`) ใช้ `chart-viewport.ts` + `chart-time-axis.ts` ร่วมกัน — เพิ่มกราฟแบบเดียวกันให้ต่อยอดจากสองไฟล์นี้ อย่าเขียน pointer/pinch ใหม่. เส้นโค้งใช้ `svgPathFromPoints`/`svgLine` ตัวเดียวทั้ง server และ client script, สีอ้าง `var(--chart-*)`, แนบ legend ด้วย `LegendRow` + `.legend-dot`
