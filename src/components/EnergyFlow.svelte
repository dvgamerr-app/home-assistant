<script lang="ts">
  ...
  onMount(() => {
    ...
    socket.on('live', (data: LiveSnapshot) => {
      pv = data.pvPowerKw
      load = data.loadPowerKw
      battPower = data.batteryPowerKw
      soc = data.batterySoc
...
<section class="flex h-full flex-col rounded-lg border border-border/70 bg-card p-6 md:p-8">
  ...
  <svg bind:this={svgEl} viewBox="0 0 360 380" class="h-auto w-full" role="img" aria-label="แผนผังการไหลของพลังงาน">
    <defs>
    ...
    {#if charging}{@render dots('p-hb', 'var(--chart-2)', 3, battPower)}{/if}
