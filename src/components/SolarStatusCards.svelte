<script lang="ts">
  ...
  // PV strings derived
  // fallback เฉลี่ยตามจำนวนชุด ใช้เฉพาะตอน capacityKw ยังไม่มา — แบ่งเท่ากันไม่ตรงกับ layout 10/5 จริง
  ...
  <section class="rounded-lg border border-border/70 bg-card p-6">
    ...
    <div class="space-y-5">
      {#each pvStrings as pv, i (pv.name)}
        {@const lp = pvLive(i)}
        {@const maxRef = pv.capacityKw > 0 ? pv.capacityKw : perStringMax}
        ...
        <div>
          ...
          <div class="mt-1.5 flex items-center justify-between text-[11px] text-muted-foreground">
            ...
            <span>{pv.panels} แผง · {num(maxRef, 2)} kWp</span>
          </div>
        </div>
      {/each}
    </div>
